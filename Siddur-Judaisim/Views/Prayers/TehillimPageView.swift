...
struct TehillimPageView: View {
    ...
    // Safe access to current episode — prevents index-out-of-bounds
    private var currentEpisode: TehillimEpisode? {
        guard selectedTehillimEpisode >= 0 && selectedTehillimEpisode < episodeGroup.count else { return nil }
        return episodeGroup[selectedTehillimEpisode]
    }
    ...
    private var favoriteButton: some View {
        Button(action: {
            if let ep = currentEpisode {
                TehillimModel.toggleFavorite(for: ep.id)
            }
        }) {
            if let ep = currentEpisode {
                Image(systemName: TehillimModel.isFavorite(episodeId: ep.id) ? "star.fill" : "star")
                    .foregroundStyle(CustomPalette.golden.color)
            }
        }
    }
...
//                    .font(.custom("Guttman Drogolin-Bold", size: textSize))
                    .font(.custom(appSettings.selectedFontName, size: appSettings.textSize))
